...
case class CondensedFixtureListView(
  fixtureList: Seq[(CalendarDay, Map[TournamentRound, Seq[Fixture]])],
  teamId: Int
) extends View {
  ...
  private val fixturesElements = fixtureList.flatMap {
    case (day, rounds) =>
      rounds.flatMap {
        case (round, fixtures) =>
          fixtures
            .filter(f => f.host.id == teamId || f.visitor.id == teamId)
            .map { fixture =>
              div(
                ...
                div(
                  ...
                  div(
                    `class` := "col-xs-2",
                    span(day.date.toString + " - " + day.date.toWeekDay(false))
                  ),
